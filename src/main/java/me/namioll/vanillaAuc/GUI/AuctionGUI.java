...
public class AuctionGUI {
    ...
    private static final long REMAINED = 3L * 24 * 60 * 60 * 1000;
    ...
    public static Inventory createAuctionGUI(List<AuctionItem> lots, int page){
        ...
        updateAuctionGUI(inv, lots, page);

        return inv;
    }

    public static void updateAuctionGUI(Inventory inv, List<AuctionItem> lots, int page) {
        int lastpage = lots.isEmpty() ? 0 : (lots.size() - 1) / ITEM_SLOTS.length;
        ...
        loreVpered.add(mm.deserialize("<!italic><gray>Страница <white>" + (page + 2) + "</white> из <white>" + (lastpage + 1)));
        ...
        loreNazad.add(mm.deserialize("<!italic><gray>Страница <white>" + page + "</white> из <white>" + (lastpage + 1)));
        metarnazad.lore(loreNazad);
        nazad.setItemMeta(metarnazad);

        ItemStack gray_glass = new ItemStack(Material.GRAY_STAINED_GLASS_PANE);
        ItemMeta glassMeta = gray_glass.getItemMeta();
        glassMeta.displayName(Component.empty());
        gray_glass.setItemMeta(glassMeta);

        inv.setItem(48, (page > 0) ? nazad : gray_glass);
        inv.setItem(50, (page < lastpage) ? vpered : gray_glass);

        int startIndex = page * 28;

        for (int i = 0; i < ITEM_SLOTS.length; i++) {
            int lotIndex = startIndex + i;
            int currentSlot = ITEM_SLOTS[i];
            ...
            //на всякий случай
            String timeString = timeLeft <= 0 ? "Истекло" : formatTime(timeLeft);

            List<Component> lore = new ArrayList<>();
            if (metaitem.hasLore()) lore.addAll(metaitem.lore());
